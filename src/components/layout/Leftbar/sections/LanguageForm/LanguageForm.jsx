import { useState } from "react";
import nextIconGray from "../../../../../assets/next.svg";
import nextIconWhite from "../../../../../assets/nextwhite.svg";

const emptyLanguage = () => ({ language: "", level: "" });

export default function LanguageForm() {
  const [items, setItems] = useState([emptyLanguage()]);

  const languages = [
    "English",
    "Spanish",
    "French",
    "German",
    "Chinese",
    "Arabic",
    "Hindi",
    "Other",
  ];

  const levels = ["Basic", "Conversational", "Fluent", "Excellent", "Native"];

  const update = (idx, key, value) => {
    setItems((prev) => prev.map((it, i) => (i === idx ? { ...it, [key]: value } : it)));
  };

  const add = () => setItems((prev) => [...prev, emptyLanguage()]);
  const remove = (idx) => setItems((prev) => prev.filter((_, i) => i !== idx));

  const isValid = items.length > 0 && items.every((it) => it.language && it.level);

  const onNext = (e) => {
    e.preventDefault();
    alert("Next Step (UI placeholder)");
  };

  return (
    <div className="flex flex-col w-full">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-gray-200 mb-6 pb-5">
        <div className="flex flex-col gap-1">
          <h2 className="text-base sm:text-lg font-semibold text-gray-800 leading-tight">Language</h2>
          <p className="text-sm text-gray-500">Fill details here</p>
        </div>
      </div>

      <form onSubmit={onNext} className="space-y-6">
        {items.map((it, idx) => (
          <div key={idx} className="bg-white border border-gray-200 rounded-lg p-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
              <div className="flex flex-col">
                <label className="text-sm font-medium text-gray-700 mb-1">Languages spoken <span className="text-red-500">*</span></label>
                <select
                  value={it.language}
                  onChange={(e) => update(idx, "language", e.target.value)}
                  className="border border-gray-300 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-[#00318B] focus:outline-none"
                >
                  <option value="">Select language</option>
                  {languages.map((l) => (
                    <option key={l} value={l}>{l}</option>
                  ))}
                </select>
              </div>

              <div className="flex flex-col">
                <label className="text-sm font-medium text-gray-700 mb-1">Proficiency level <span className="text-red-500">*</span></label>
                <select
                  value={it.level}
                  onChange={(e) => update(idx, "level", e.target.value)}
                  className="border border-gray-300 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-[#00318B] focus:outline-none"
                >
                  <option value="">Select level</option>
                  {levels.map((lvl) => (
                    <option key={lvl} value={lvl}>{lvl}</option>
                  ))}
                </select>
              </div>
            </div>

            <div className="flex items-center justify-between mt-4">
              <div>
                <button type="button" onClick={() => remove(idx)} className="text-sm text-red-500">Delete</button>
              </div>
              <div>
                {idx === items.length - 1 && (
                  <button type="button" onClick={add} className="text-sm text-blue-600">+ Add more</button>
                )}
              </div>
            </div>
          </div>
        ))}

        <div className="flex justify-end">
          <button
            type="submit"
            disabled={!isValid}
            className={`flex items-center gap-2 px-6 py-2 text-sm font-medium rounded-lg transition-colors duration-200 focus:outline-none ${isValid
              ? "bg-[#00318B] text-white hover:bg-blue-900 cursor-pointer"
              : "bg-gray-300 text-gray-500 cursor-not-allowed"
              }`}
          >
            Next
            <img src={isValid ? nextIconWhite : nextIconGray} alt="Next" className="w-4 h-4" />
          </button>
        </div>
      </form>
    </div>
  );
}
